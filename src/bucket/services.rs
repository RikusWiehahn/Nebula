...
pub async fn get_record_list(
    RecordListRequest { page, page_size }: RecordListRequest,
) -> RecordListResponse {
    let mut res = RecordListResponse::default();
    let is_admin_res = caller_is_admin();
    if is_admin_res.is_err() {
        res.err = is_admin_res.err().unwrap();
        return res;
    ...
    STATE.with(|state: &GlobalState| {
        ...
        for (_, record) in records_ref.iter() {
    ...
    for (i, record) in records.iter().enumerate() {
        ...
        page_records.push(record.clone());
    }
